...
## Algorithm
1.Load the weather dataset, clean the data by sorting timestamps, handling missing values using interpolation, and perform feature engineering such as time-based features and lag features.

2.Select relevant features (humidity, pressure, wind speed, illumination, CO₂, time features, and lag values) and split the dataset into training and testing sets based on chronological order.

3.Train Random Forest Regression models for each target variable (temperature, PM2.5, and solar radiation) using the training data and evaluate their performance using metrics like R² and MAE.

4.Visualize the results using prediction graphs and feature importance plots, and use the trained models to predict the next weather values based on the latest sensor data.
